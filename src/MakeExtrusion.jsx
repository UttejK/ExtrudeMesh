...
export class BoundingBoxGizmo extends Gizmo {
  ...
  setColor(color) {
    ...
    this._hoverColoredMaterial.emissiveColor = color
      .clone()
      .add(new Color3(0.3, 0.3, 0.3));
  ...
  constructor(
    ...
    this._hoverColoredMaterial = new StandardMaterial(
      "",
      gizmoLayer.utilityLayerScene
    );
    ...
    lines.push(
      CreateLines(
        "lines",
        {
          points: [
            new Vector3(0, 0, 0),
            new Vector3(this._boundingDimensions.x, 0, 0),
          ],
        },
        gizmoLayer.utilityLayerScene
      )
    ...
    lines.push(
      CreateLines(
        "lines",
        {
          points: [
            new Vector3(0, 0, 0),
            new Vector3(0, this._boundingDimensions.y, 0),
          ],
        },
        gizmoLayer.utilityLayerScene
      )
    ...
    lines.push(
      CreateLines(
        "lines",
        {
          points: [
            new Vector3(0, 0, 0),
            new Vector3(0, 0, this._boundingDimensions.z),
          ],
        },
        gizmoLayer.utilityLayerScene
      )
      ...
      CreateLines(
        "lines",
        {
          points: [
            new Vector3(this._boundingDimensions.x, 0, 0),
            new Vector3(
              this._boundingDimensions.x,
              this._boundingDimensions.y,
              0
            ),
          ],
        },
    ...
    lines.forEach((l) => {
      l.color = color;
      l.position.addInPlace(
        new Vector3(
          -this._boundingDimensions.x / 2,
          -this._boundingDimensions.y / 2,
          -this._boundingDimensions.z / 2
        )
      );
      l.isPickable = false;
    ...
    this._rotateSpheresParent = new AbstractMesh(
      "",
      gizmoLayer.utilityLayerScene
    );
    ...
    for (let i = 0; i < 12; i++) {
      ...
      _dragBehavior.onDragObservable.add((event) => {
        ...
        if (this.attachedMesh) {
          ...
          if (
            originalParent &&
            originalParent.scaling &&
            originalParent.scaling.isNonUniformWithinEpsilon(0.001)
          ) {
            Logger.Warn(
          ...
          const toSub = event.dragPlaneNormal.scale(
          ...
          let projectDist =
            Vector3.Dot(dragAxis, event.delta) < 0
              ? Math.abs(event.delta.length())
              : -Math.abs(event.delta.length());
          // Make rotation relative to size of mesh.
          projectDist =
            (projectDist / this._boundingDimensions.length()) *
            this._anchorMesh.scaling.length();
          ...
          if (!this._anchorMesh.rotationQuaternion) {
            this._anchorMesh.rotationQuaternion =
              Quaternion.RotationYawPitchRoll(
          ...
          if (Math.abs(totalTurnAmountOfDrag) <= 2 * Math.PI) {
            ...
            this._anchorMesh.rotationQuaternion.multiplyToRef(
              this._tmpQuaternion,
              this._anchorMesh.rotationQuaternion
            );
            this._anchorMesh.removeChild(
              this.attachedMesh,
        ...
        for (let k = 0; k < 3; k++) {
          ...
          _dragBehavior.onDragObservable.add((event) => {
            ...
            if (this.attachedMesh) {
              ...
              PivotTools._RemoveAndStorePivotPoint(this.attachedMesh);
              const relativeDragDistance =
                (event.dragDistance / this._boundingDimensions.length()) *
                this._anchorMesh.scaling.length();
              ...
              if (this.scalePivot) {
                this.attachedMesh
                  .getWorldMatrix()
                  .getRotationMatrixToRef(this._tmpRotationMatrix);
                ...
                Vector3.TransformCoordinatesToRef(
                  this._tmpVector,
                  this._tmpRotationMatrix,
                  this._tmpVector
                );
                this._anchorMesh.position.subtractInPlace(this._tmpVector);
                this._boundingDimensions.multiplyToRef(
                  this.scalePivot,
                  this._tmpVector
                );
                Vector3.TransformCoordinatesToRef(
                ...
                box.absolutePosition.subtractToRef(
                  this._anchorMesh.position,
                  this._tmpVector
                );
                this._anchorMesh.position.subtractInPlace(this._tmpVector);
              }
              this._anchorMesh.addChild(
                this.attachedMesh,
                Gizmo.PreserveScaling
              );
              this._anchorMesh.scaling.addInPlace(deltaScale);
              if (
                this._anchorMesh.scaling.x < 0 ||
                this._anchorMesh.scaling.y < 0 ||
                this._anchorMesh.scaling.z < 0
              ) {
        ...
        if (!pointerIds[pointerInfo.event.pointerId]) {
          ...
          if (
            ...
            pointerInfo.pickInfo.pickedMesh !=
              pointerIds[pointerInfo.event.pointerId]
          ) {
            pointerIds[pointerInfo.event.pointerId].material =
    ...
    this._renderObserver =
      this.gizmoLayer.originalScene.onBeforeRenderObservable.add(() => {
        ...
        if (
          this.attachedMesh &&
          !this._existingMeshScale.equals(this.attachedMesh.scaling)
        ) {
          this.updateBoundingBox();
        } else if (
        ...
        if (
          this._dragMesh &&
          this.attachedMesh &&
          this._pointerDragBehavior.dragging
        ) {
          this._lineBoundingBox.position.rotateByQuaternionToRef(
            this._rootMesh.rotationQuaternion,
            this._tmpVector
          );
          this.attachedMesh.setAbsolutePosition(
            this._dragMesh.position.add(this._tmpVector.scale(-1))
          );
        }
      });
    this.updateBoundingBox();
  ...
  _unhoverMeshOnTouchUp(pointerInfo, selectedMesh) {
    ...
    if (
      (pointerInfo === null || pointerInfo === void 0
        ? void 0
        : pointerInfo.event) instanceof PointerEvent &&
      (pointerInfo === null || pointerInfo === void 0
        ? void 0
  ...
  updateBoundingBox() {
    if (this.attachedMesh) {
      ...
      this._anchorMesh.rotationQuaternion.copyFrom(
        this.attachedMesh.rotationQuaternion
      );
      ...
      boundingMinMax.max.subtractToRef(
        boundingMinMax.min,
        this._boundingDimensions
      );
      ...
      this._rotateSpheresParent.position.copyFrom(
        this._lineBoundingBox.position
      );
      this._scaleBoxesParent.position.copyFrom(this._lineBoundingBox.position);
      this._lineBoundingBox.computeWorldMatrix();
      this._anchorMesh.position.copyFrom(
        this._lineBoundingBox.absolutePosition
      );
        ...
        for (let k = 0; k < 2; k++) {
          ...
          if (i == 0) {
            rotateSpheres[index].position.set(
              this._boundingDimensions.x / 2,
              this._boundingDimensions.y * j,
              this._boundingDimensions.z * k
            );
          ...
          if (
            ...
            const distanceFromCamera =
              (this.rotationSphereSize * this._tmpVector.length()) /
              this.fixedDragMeshScreenSizeDistanceFactor;
        ...
        for (let k = 0; k < 3; k++) {
          ...
          if (scaleBoxes[index]) {
            scaleBoxes[index].position.set(
              ...
              this._boundingDimensions.y * (j / 2),
              this._boundingDimensions.z * (k / 2)
            );
            ...
            if (
              ...
              const distanceFromCamera =
                (this.scaleBoxSize * this._tmpVector.length()) /
                this.fixedDragMeshScreenSizeDistanceFactor;
